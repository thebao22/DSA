...
void removeBefore(List *&L, int val)
{
    ...
    NODE *temp = L->p_head;
    while(cur -> p_next != nullptr && cur -> p_next ->key != val){
        cur = cur -> p_next;
        temp = temp->p_next;
    }
    if(cur -> p_next != nullptr && cur -> p_next ->key == val){
...
void removeAfter(List *&L, int val)
{
    ...
    if( L->p_head ->key == val){
        ...
        L->p_head -> p_next = L->p_head->p_next->p_next;
        if (temp == L->p_tail) L->p_tail = L->p_head; // cap nhat lai node tail
        delete temp;
        return;
    }
    ...
    if(cur != nullptr && cur->key == val && cur -> p_next != nullptr){
...
bool addPos(List *&L, int data, int pos)
{
    if(pos < 0 || pos > countElements(L)) return false;
    NODE *newnode = createNode(data);
    if (newnode == nullptr) return false;
    ...
    NODE *cur = L->p_head;
    for(int i = 0; i<pos-1 && cur -> p_next != nullptr; i++){
        cur = cur ->p_next;
    }
    newnode->p_next = cur -> p_next;
    cur->p_next  = newnode;
    if (newnode->p_next == nullptr) L->p_tail = newnode;
    return true;
}

void removePos(List *&L, int data, int pos)
{
    if(pos < 0 || pos > countElements(L)) return;
    if(pos == 0 ) {
        removeHead(L);
        return;
    }
...
List *reverseList(List *L)
{
    ...
    if(L->p_head == nullptr) return newlist;
    NODE *read = L->p_head;
    while(read!=nullptr){
        addHead(newlist,read->key);
        read = read -> p_next;
    }
    delete read;
    return newlist;
...
void removeDuplicate(List *&L)
{
    ...
    while(cur != nullptr){
        ...
        check = cur->p_next;
        while(check!=nullptr){
            if(read->key == check->key){
                NODE *newnode = check;
                read->p_next = check->p_next;
                check = check ->p_next;
                if(newnode == L->p_tail) L->p_tail = read;
                delete newnode;
            }
            else{
                read=check;
...
bool removeElement(List *&L, int key)
{
    ...
    NODE *cur = L->p_head;
    while(cur ->p_next != nullptr && cur->p_next->key != key){
        cur = cur->p_next;
    }
    if(cur ->p_next != nullptr && cur->p_next->key == key){
        removeAfter(L,cur->key);
        return true;
    }
    return false;
