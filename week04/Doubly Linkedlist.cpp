...
void removeBefore(List *&L, int val) {
    ...
    if (cur && cur->p_prev) {
        NODE *temp = cur->p_prev;
        if (temp == L->p_head) {
            L->p_head = cur;
            L->p_head->p_prev = nullptr;
        } else {
            temp->p_prev->p_next = cur;
            cur->p_prev = temp->p_prev;
        }
        delete temp;
    }
}
...
bool addPos(List *&L, int data, int pos) {
    ...
    if (pos == count) return addTail(L, data);
    NODE *newnode = createNode(data);
    if (!newnode) return false;
    NODE *cur = L->p_head;
